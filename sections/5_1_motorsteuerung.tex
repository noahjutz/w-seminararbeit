...
\[
    \text{steps} = \frac{\text{measurementRange}}{\text{mmPerRevolution}} \cdot \text{stepsPerRevolution}
\]
...
int steps = (float(measurementRange) / mmPerRevolution) * stepsPerRevolution;
stepper.step(steps);
stepper.step(-steps);
\end{lstlisting}
